...
def calculate_staff_table(
    ...
    project_duration = calculate_year_fractions(
    ...
    in_kind_staff_costs = calculate_column_total(
        in_kind_staff_costs, start_year, end_year
    )
...
def calculate_column_total(
    ...
    """
    column_total = {
        year: sum(row["results"].get(year) or 0 for row in data.values())
    ...
    # Add result into input data dictionary
    data["column_total"] = {"results": column_total, "total": total}
...
def calculate_staff_row(
    ...
    year_employed = 0
    for year in range(project_duration["start_year"], project_duration["end_year"] + 1):
        # Skip none value
