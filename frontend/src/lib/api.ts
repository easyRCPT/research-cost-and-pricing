...
// ---------------------------------------------------------------------------
// Project — created once. Identity/admin info only (models.Project).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Budget — one costed attempt against a Project. A project can have several
// (a revision, a variant for a different funder) — models.Budget.
// ---------------------------------------------------------------------------

export type BudgetMode = "simple" | "full"

/** Matches Budget.Status choices exactly. No "rejected" — withdrawn instead. */
...
export interface BudgetRecord {
  ...
  /**
   * Costing/pricing form data (staff rows, non-staff rows, dates, etc).
   * Reuses rcpt-engine.ts's existing `Project` shape as-is — that type is
   * the one-budget-worth-of-line-items shape, distinct from the backend's
   * `Project` model above, which is why it's imported under an alias here.
   */
  data: CostingData

  /** Last computed pricing. Null until the first calculate/submit call. */
  priceSummary: PriceSummary | null
  /** Client-only checks not yet ported to the backend. See file header. */
  frontendChecks: FrontendChecks | null
}

// ---------------------------------------------------------------------------
// PriceSummary — matches calculate_price_summary()'s return dict exactly,
// field for field, so this needs no translation once the real endpoint exists.
// ---------------------------------------------------------------------------
